...
class TigerHash
{
    uint64_t h0=0x0123456789abcdef;
    uint64_t h1=0xfedcba976543210;
    uint64_t h2=0xf096a5b43d2e187;
    constexpr static const uint64_t t1[256] =
        {
    ...
    //S-box 2
    constexpr static const uint64_t t2[256] =
        {
    ...
    //S-box 3
    constexpr static const uint64_t t3[256] =
        {
    ...
    //S-box 4
    constexpr static const uint64_t t4[256] =
        {
    ...
    std::vector<uint8_t> preprocess(string s) {
        uint64_t len=s.length();
        int paddedLength=((len+1+8+63)/64)*64;
        vector<uint8_t>processed(paddedLength,0);
        memcpy(processed.data(),s.c_str(),len);
        processed[len]=0x01;
        len=len*8;
        memcpy(processed.data()+paddedLength-8,&len,8);
        return processed;
    }
    vector<uint8_t> tigerHash(string& message) {
        auto processed=preprocess(message);
        uint64_t hh0=h0,hh1=h1,hh2=hh2;
        for(int i=0;i<processed.size();i+=64) {
            uint64_t w[8];
            memcpy(w,processed.data()+i,64);
            uint64_t a=hh0,b=hh1,c=hh2;
            for(int j=0;j<4;j++) {
                for(int k=0;k<8;k++) {
                    c=c^w[k];
                    uint8_t mc[8];
                    memcpy(mc,&c,8);
                    a=a-(t1[mc[0]] ^ t2[mc[2]] ^ t3[mc[4]] ^ t4[mc[6]]);
                    b=b + (t4[mc[1]]^ t3[mc[3]] ^ t2[mc[5]] ^ t4[mc[7]]);
                    b=b *(i+1);
                }
                if (j==0) {
        ...
        vector<uint8_t>res(24,0);
        memcpy(res.data(),&hh0,8);
        memcpy(res.data()+8,&hh1,8);
        memcpy(res.data()+16,&hh2,8);
        return res;

    }
public:
    TigerHash();
    string hashTiger(string d) {
        auto input=preprocess(d);
        auto hash=tigerHash(d);
        stringstream s;
        for (auto byte : hash) {
            s << hex << setw(2) << setfill('0') << (int)byte;
        }
        return s.str();
    }
};
